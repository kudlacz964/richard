...
def resize(img):
    desired_dimension = 256
    current_width = img.shape[0]
    current_height = img.shape[1]
    width = current_width / desired_dimension
    height = current_height / desired_dimension
...
def prepare_png(dataset, folder_name, channels=(0, 1, 2), crop=False):
    ...
    image_dirs = {
        "input": os.path.join(data_path, "tmp"),
    ...
    #output_path = os.path.join(data_path, f"{folder_name}")
    ...
    for image_name in tqdm(os.listdir(image_dirs[dataset])):
        ds = pydicom.dcmread(os.path.join(image_dirs[dataset], image_name), force = True)
        ...
        im.save(os.path.join(output_path, image_name[:-4] + ".jpg"))
...
def prepare_png_adjacent(dataset, folder_name, crop=True):
    ...
    triplet_dfs = {
        "DICOMs": os.path.join(data_path, "DICOMs"),
...
if __name__ == '__main__':
    prepare_png("input", "output", channels=(1, 2, 6), crop=False)
